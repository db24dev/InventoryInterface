"use client";
import Image from "next/image";
import { useState } from 'react';
export default function LandingPage() {
  const [email, setEmail] = useState('');
  const [message, setMessage] = useState('');

  const handleSubmit = async (e: { preventDefault: () => void; }) => {
    e.preventDefault();
    setMessage("Thank you for signing up! We'll keep you posted.");
    setEmail('');
  };

  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-white text-black p-6">
      {/* Hero Section */}
      <div className="text-center max-w-4xl w-full p-10">
        <Image className='inline w-20 rounded-full h-20 object-cover border-2 border-black' src="./STOCKEFY.jpg"></Image>
        <h1 className="text-6xl font-bold">Welcome to STOCKEFY</h1>
        <p className="text-2xl mt-4">Your Ultimate Inventory Management Solution</p>
        <p className="text-lg mt-2 text-gray-400">
          Streamline your inventory for Shopify, TikTok, and Instagram sellers.
        </p>
      </div>
      
      {/* Video Embed */}
      <div className="mt-6 w-full max-w-3xl">
        <iframe
          className="w-full h-64 rounded-lg"
          src="https://www.youtube.com/embed/YOUR_YOUTUBE_VIDEO_ID"
          title="Stockefy Introduction"
          allowFullScreen
        ></iframe>
      </div>
      
      {/* Steps Section */}
      <div className="mt-10 text-center text-white max-w-3xl">
        <h2 className="text-3xl font-semibold">How It Works</h2>
        <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="p-6 bg-gray-800 rounded-lg shadow-lg">
            <h3 className="text-xl font-semibold">Step 1</h3>
            <p>Choose the Membership Tier more adequate to your business needs.</p>
          </div>
          <div className="p-6 bg-gray-800 rounded-lg shadow-lg">
            <h3 className="text-xl font-semibold">Step 2</h3>
            <p>Sign-Up/Payment/Create Account.</p>
          </div>
          <div className="p-6 bg-gray-800 rounded-lg shadow-lg">
            <h3 className="text-xl font-semibold">Step 3</h3>
            <p>Navigate the Platform: Choose and customize features for your business.</p>
          </div>
        </div>
      </div>
      
      {/* Email Signup Form */}
      <div className="mt-10 text-center max-w-lg w-full">
        <h2 className="text-2xl font-semibold">Stay Ahead of Your Competition</h2>
        <form onSubmit={handleSubmit} className="mt-6">
          <input
            type="email"
            placeholder="Enter your email"
            value={email}
            onChange={(e) => setEmail(e.target.value)}
            className="w-full p-3 rounded-md text-black border-2 border-black"
            required
          />
          <button type="submit" className="mt-4 w-full bg-blue-500 hover:bg-blue-600 text-white py-2 rounded-md">
            Get Early Access
          </button>
        </form>
        {message && <p className="mt-3 text-sm text-green-400">{message}</p>}
      </div>
      
      {/* Membership Tiers */}
      <div className="mt-10 text-center text-white max-w-4xl">
        <h2 className="text-3xl font-semibold">Choose Your Plan</h2>
        <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="p-6 bg-gray-800 rounded-lg shadow-lg">
            <h3 className="text-xl font-semibold">Basic ($19/mo)</h3>
            <p>Core dashboard + analytics</p>
          </div>
          <div className="p-6 bg-gray-800 rounded-lg shadow-lg">
            <h3 className="text-xl font-semibold">Pro ($49/mo)</h3>
            <p>Advanced analytics + white-labeling</p>
          </div>
          <div className="p-6 bg-gray-800 rounded-lg shadow-lg">
            <h3 className="text-xl font-semibold">Enterprise ($99/mo)</h3>
            <p>Full customization + integrations</p>
          </div>
        </div>
      </div>
      
      {/* Social Media Links */}
      <div className="mt-10 flex space-x-6">
      <div className='w-6 bg-white'><a href="https://x.com/Stockefy" className="text-blue-400 hover:underline"><Image src="data:image/svg+xml;base64,"/></a></div>
          <div className='w-6'><a href="https://www.instagram.com/stockefy.io/" className="text-blue-400 hover:underline"><Image src="data:image/svg+xml;base64,"/></a></div>
          <div className='w-6'><a href="https://twitter.com/Stockefy" className="text-blue-400 hover:underline"><Image src="data:image/svg+xml;base64,"/></a></div>
          <div className='w-6 bg-white'><a href="https://twitter.com/Stockefy" className="text-blue-400 hover:underline"><Image src="data:image/svg+xml;base64,"/></a></div>
          <div className='w-6'><a href="https://twitter.com/Stockefy" className="text-blue-400 hover:underline"><Image src="data:image/svg+xml;base64,"/></a></div>
      </div>
    </div>
  );
}
